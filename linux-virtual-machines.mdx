...
<script type="application/ld+json" dangerouslySetInnerHTML={{
  __html: JSON.stringify({
    ...
    "@type": "TechArticle",
    "headline": "Complete Guide to Running Virtual Machines on Vast.ai",
    "description": "Comprehensive technical guide to running VM instances on Vast.ai using KVM, including advantages, limitations, use cases, configuration requirements, and best practices for VM-based GPU computing.",
    "author": {
      "@type": "Organization",
      "name": "Vast.ai"
    },
    "articleSection": "Virtual Machines Documentation",
    "keywords": ["virtual machines", "KVM", "VMs", "Docker", "systemd", "GPU", "CUDA", "development", "vast.ai"]
  })
}} />

<Warning>
VM instances currently have a ~25% creation failure rate and longer boot times (5–30 minutes) compared to Docker-based instances. If your workload doesn't require init managers, nested containers, or GPU passthrough, use a [Docker-based instance](/documentation/instances/docker-environment) instead.
</Warning>
...
- Use cli command: `vastai copy $SRC_VM_ID $DEST_VM_ID`
- Note: `vastai vm copy` is not available as a CLI command — use `vastai copy` instead
- Limitations:
  - Only supports full VM migration (full disk replacement, no subpaths)
