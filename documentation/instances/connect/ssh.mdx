...
<Warning>
**Register your SSH key before creating an instance.** Account-level SSH keys
(registered via `POST /api/v0/ssh/` or `vastai create ssh-key`) are applied at
container creation time.

If you forgot: use the SSH key button on the instance card in the console to add a key
to a running instance — no need to destroy and recreate.
</Warning>
