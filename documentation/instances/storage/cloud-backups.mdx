...
<script type="application/ld+json" dangerouslySetInnerHTML={{
  __html: JSON.stringify({
    ...
    "@type": "HowTo",
    "name": "How to Set Up Scheduled Cloud Backups on Vast.ai",
    "description": "A comprehensive guide to setting up and managing automated cloud backups for Vast.ai Docker instances using CLI scheduled jobs or cron, including viewing and deleting scheduled jobs.",
    "step": [
      {
        "@type": "HowToStep",
        "name": "Set Up Cloud Storage Connection",
        "text": "Before creating backup jobs, set up a cloud storage connection (Google Drive, S3, Backblaze, or Dropbox) in Vast.ai Settings page at cloud.vast.ai/account. View existing connections using 'vastai show connections'."
      },
      {
        "@type": "HowToStep",
        "name": "Create Scheduled Backup via CLI",
        "text": "Use vastai cloud copy command with schedule parameters. For weekly backups: 'vastai cloud copy --src /workspace --dst /backups/ID_backups/ --instance ID --connection CONNECTION_ID --transfer \"Instance To Cloud\" --schedule WEEKLY --day 6 --hour 21'. For daily: use --schedule DAILY --day \"*\". For hourly: use --schedule HOURLY --day \"*\" --hour \"*\"'."
      },
      {
        "@type": "HowToStep",
        "name": "Create Backup via Cron (Alternative)",
        "text": "For more granular control, use cron on your local Linux/Mac computer. Open crontab with 'crontab -e' and add a line like '0 */4 * * * vastai cloud copy --src /workspace --dst /backups/ID/ --instance ID --connection CONNECTION_ID --transfer \"Instance To Cloud\"' for backups every 4 hours."
      },
      {
        "@type": "HowToStep",
        "name": "View Scheduled Backup Jobs",
        "text": "View all scheduled backup jobs with 'vastai show scheduled-jobs'. The output shows Scheduled Job ID, Instance ID, API Endpoint, start/end dates in UTC, day of week, hour of day in UTC, minute of hour, and frequency (HOURLY, DAILY, WEEKLY)."
      },
      {
        "@type": "HowToStep",
        "name": "Delete Scheduled Job (Optional)",
        "text": "To remove a scheduled backup job, use 'vastai delete scheduled-job JOB_ID'. Find the job ID by running 'vastai show scheduled-jobs' first. Successful deletion returns a confirmation message."
      }
    ]
  })
}} />
...
```text
vastai cloud copy --src /workspace --dst /backups/19015821_backups/ --instance 19015821 --connection 19447 --transfer "Instance To Cloud" --schedule WEEKLY --day 6 --hour 21
``` 
...
```text
vastai cloud copy --src /workspace --dst /backups/19015821_backups/ --instance 19015821 --connection 19447 --transfer "Instance To Cloud" --schedule DAILY --day "*" --hour 21
```
...
```text
vastai cloud copy --src /workspace --dst /backups/19015821_backups/ --instance 19015821 --connection 19447 --transfer "Instance To Cloud" --schedule HOURLY --day "*" --hour "*"
```
...
```text
0 */4 * * * vastai cloud copy --src /workspace --dst /backups/19015821_backups/ --instance 19015821 --connection 19447 --transfer 'Instance To Cloud'
```
...
```bash Bash
vastai delete scheduled-job 4462309
```
