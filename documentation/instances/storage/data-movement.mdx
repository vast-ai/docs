...
"canonical": "/documentation/instances/storage/data-movement"
---
...
<script type="application/ld+json" dangerouslySetInnerHTML={{
  __html: JSON.stringify({
    ...
    "@type": "HowTo",
    "name": "How to Move Data on Vast.ai",
    "description": "A comprehensive guide to moving data on Vast.ai using various methods including cloud sync, instance-to-instance transfers, CLI copy commands, VM migration, and SCP, with performance considerations and best practices.",
    "step": [
      {
        "@type": "HowToStep",
        "name": "Use Cloud Sync for Cloud Storage",
        "text": "Set up cloud storage connections (S3, Google Drive, Backblaze, Dropbox) on the account page at cloud.vast.ai/account. Use the cloud copy button on instance cards in the GUI, or use the 'vastai cloud copy' CLI command. Works even when instances are stopped. See Cloud Sync documentation for details."
      },
      {
        "@type": "HowToStep",
        "name": "Copy Between Instances via GUI",
        "text": "For Docker instances: Click copy button on source instance, then on destination instance. Pick source and destination folders (leave '/' at end of source to copy contents vs entire directory). Avoid copying to /root or / as destination. Works with stopped/inactive instances. For VM instances: Use confirmation dialog to copy entire source instance to destination (replaces destination disk)."
      },
      {
        "@type": "HowToStep",
        "name": "Use CLI Copy Command",
        "text": "Use 'vastai copy' for Docker instances between remote and local, or between two remotes. Format: 'vastai copy [instance_id:]path [instance_id:]path' or structured syntax like 'C.instance_id:path' for containers, 'cloud_service:path' for cloud, 'local:path' for local. Examples: 'vastai copy 6003036:/workspace/ 6003038:/workspace/', 'vastai copy C.11824:/data/test local:data/test'."
      },
      {
        "@type": "HowToStep",
        "name": "Migrate VMs with CLI",
        "text": "For VM instances, use 'vastai copy SOURCE_ID DEST_ID' to transfer entire VM contents to another instance. The destination instance must be stopped during transfer. The destination VM's disk will be completely replaced with the source machine's contents."
      },
      {
        "@type": "HowToStep",
        "name": "Use SCP for Direct File Transfer",
        "text": "For smaller transfers (< 1GB), use SCP with format 'scp -P PORT LOCAL_FILE root@IPADDR:/REMOTEDIR'. Get PORT and IPADDR from SSH connect button. Use direct SSH connection for larger transfers. Example: 'scp -P 7417 myfile.tar.gz root@52.204.230.7:/mydir'. Note: Use uppercase -P for SCP (different from ssh's lowercase -p)."
      }
    ]
  })
}} />
...
1. Instance\<-\>Instance migration through the `vastai copy` CLI command or the GUI instance control panel
...
For VM instances, use the standard `vastai copy` command with two instance IDs. This performs a full-disk migration: the destination VM's disk is completely replaced with the contents of the source machine. No subpaths or individual folder selection is supported.

<Note>
  `vastai vm copy` is not available as a CLI command. Use `vastai copy` instead.
</Note>

Example:

```text Text
vastai copy 1241241 1241245
```

This will transfer the entire disk contents of instance 1241241 to instance 1241245.
...
### Transfer appears stuck or shows no progress

1. **Check transfer status** — Run `vastai show instances` (or check the instance card in the console). A copy operation in progress shows a status message on the destination instance.
2. **Wait for the initial handshake** — After clicking the copy button in the GUI, allow 5–10 seconds for the transfer to begin before assuming it is stuck.
3. **Verify both instances are reachable** — If either instance is in an error state, the copy will silently fail. Check that both source and destination show `running` or `stopped` (not `error`).
4. **Cancel and retry** — Use the Cancel Copy button in the GUI or the `vastai cancel copy INSTANCE_ID` CLI command. Then start a new copy operation.
5. **Check available disk space on the destination** — A transfer that starts but stops mid-way is often caused by the destination running out of disk space. Use `vastai execute DEST_ID 'df -h'` to check.

### VM copy fails or destination is corrupted

- The destination VM must be **stopped** before running `vastai vm copy SRC_ID DEST_ID`. If it is running, stop it first with `vastai stop instance DEST_ID`.
- VM copy replaces the entire destination disk — there is no incremental option. Ensure the destination has at least as much disk space as the source.

### GUI copy button is missing for VM instances

- Instance-to-instance folder copy (the folder picker dialog) is only available for Docker-based instances. For VMs, use the CLI command `vastai vm copy SRC_ID DEST_ID`, which copies the entire VM disk.

### Cloud Sync not completing

- Verify that your cloud credentials are saved in [Account Settings](https://cloud.vast.ai/settings/) and that the bucket/folder path is correct.
- Check the instance logs (`vastai logs INSTANCE_ID`) for authentication errors from the cloud provider.
- Ensure the instance has outbound internet access and sufficient disk space to buffer the transfer.
