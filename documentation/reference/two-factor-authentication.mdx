...
To use 2FA, you must first configure **at least one Authenticator App or SMS method**. Email is a backup-only verification method — it is used automatically when you set up your *first* 2FA method (since you have nothing else to verify with at that point), but it is not used at login.
...
- **Email** — A 6-digit code is sent to your registered email address. Used **only** as the verification channel when you are setting up your first 2FA method. Once you have an Authenticator App or SMS method configured, email is no longer used for any other verification (including subsequent logins).
...
1. If you have an Authenticator App method, open your authenticator app and enter the current 6-digit code.
2. If you have an SMS method, a 6-digit code is sent to your phone.
3. Enter the 6-digit code in the verification prompt.
4. If you did not receive the SMS code, click **Resend** after 30 seconds.
5. You can also enter one of your recovery codes if your phone or authenticator app is unavailable.

> **Note:** Email 2FA is not triggered at login. To use 2FA, you must have at least one Authenticator App or SMS method configured. Email is only used as the initial verification channel when you set up your *first* 2FA method.
