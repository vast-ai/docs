  ...
  __html: JSON.stringify({
    ...
    "mainEntity": [
      ...
      },
      {
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "If the test fails, the CLI will display detailed reasons for failure. Apply the fixes and rerun the test. You can also use the --ignore-requirements flag to check rentability in a relaxed mode."
        }
      },
      {
        "@type": "Question",
        ...
        "acceptedAnswer": {
          "@type": "Answer",
          "text": "The --ignore-requirements flag runs the test in a relaxed mode, bypassing some checks. Even if the test passes in this mode, your machine does not meet the minimum verification requirements. Your machine must still have at least three direct open ports."
        }
      }
    ]
  })
...
Verification confirms that your machine is stable, performant, and properly configured to run customer workloads.
...
<Tip>
**Tip:** Ensure no other jobs are running during the self-test for the most accurate results.
</Tip>
...
If your machine fails the self-test but you still want to check its rentability, you can rerun the test with the `--ignore-requirements` flag:
