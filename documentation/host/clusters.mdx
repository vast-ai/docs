...
"canonical": "/documentation/host/clusters"
---
    ...
    "step": [
      {
        ...
        "text": "Install or upgrade the Vast CLI with pip install -U vastai."
      },
      {
        "@type": "HowToStep",
      ...
      },
      {
        ...
        "text": "Run vastai create cluster IPv4SUBNET MACHINE_ID_OF_MANAGER_NODE to initialize a cluster containing the machine with the manager node ID and using the network interface corresponding to the subnet."
      },
      {
        "@type": "HowToStep",
        "name": "Verify Cluster Creation",
        "text": "Run vastai show clusters to check the ID of the cluster you just created and see its subnet, manager node machine_id, and list of member machines."
      },
      {
        "@type": "HowToStep",
        "name": "Add Member Machines",
        "text": "Run vastai join cluster CLUSTER_ID MACHINE_IDS where MACHINE_IDS is a space separated list of the remaining machines to add to your cluster."
      }
    ]
  })
}} />
...
- Install or upgrade the Vast CLI: `pip install -U vastai`.
...
- Run `vastai create cluster IPv4SUBNET MACHINE_ID_OF_MANAGER_NODE`
- Run `vastai show clusters` to check the ID of the cluster you just created.&#x20;
- Run `vastai join cluster CLUSTER_ID MACHINE_IDS` where `MACHINE_IDS` is a space seperated list of the remaining machines to add to your cluster.&#x20;
