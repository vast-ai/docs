...
- ComfyUI
- ComfyUI API Wrapper
- Stable Diffusion 1.5 (`v1-5-pruned-emaonly-fp16.safetensors`) — pre-installed for benchmarking and immediate use
...
<Note>
**Pre-installed models:** Only Stable Diffusion 1.5 is included in the base image. All other models (SDXL, Flux, ControlNet, LoRA weights, custom checkpoints, etc.) must be downloaded separately — either via the Provisioning Script, a custom Docker image, or by installing them interactively on a running instance. Reference the checkpoint filename `v1-5-pruned-emaonly-fp16.safetensors` in your workflow's `CheckpointLoaderSimple` node to use the pre-installed SD 1.5 model without any additional setup.
</Note>
