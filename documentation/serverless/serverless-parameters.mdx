...
## Minimum Workers (`cold_workers` / `min_workers`)
...
<Note>
This parameter is named `cold_workers` in the API and CLI (`--cold_workers`). It is also referred to as `min_workers` in some documentation. Use `cold_workers` when calling the API directly or using the CLI.
</Note>
