...
- `inactivity_timeout` (integer, optional): Number of seconds of inactivity after which the endpoint is allowed to scale to zero active or total workers (depending on other settings). A value of `0` disables inactivity gating; a negative value prevents scaling to zero entirely.
- `max_queue_time` (float, optional): Maximum seconds of expected queue time allowed per worker before new requests are held in the global queue.
- `target_queue_time` (float, optional): Queue time threshold (in seconds) above which the autoscaler aggressively scales up workers.
...
## Example: Creating an Endpoint with the Vast CLI
  ...
  ```none Terminal
  vastai create endpoint --min_load 10 --target_util 0.9 --cold_mult 2.0 --cold_workers 5 --max_workers 20 --inactivity_timeout 300 --max_queue_time 30 --target_queue_time 10 --endpoint_name "my-endpoint"
  ```
