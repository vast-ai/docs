...
- `PYTORCH_CUDA_ALLOC_CONF`(string): Controls the CUDA memory allocator. Set to `expandable_segments:True` to reduce memory fragmentation on GPUs with 24 GB VRAM or less.

<Tip>
**Tip for 24 GB GPUs (RTX 3090, RTX 4090, etc.):** Larger models such as Codestral-22B or LLaMA-3-22B can trigger CUDA out-of-memory errors caused by memory fragmentation rather than total memory exhaustion. Setting `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True` in your environment variables resolves this in most cases. Add it to your Vast account environment variables so it applies automatically to every instance you launch.
</Tip>
