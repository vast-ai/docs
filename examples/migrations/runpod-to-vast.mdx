...
RunPod's "Docker Command" field in template settings passes arguments to the container's ENTRYPOINT — in the REST API this is `"dockerStartCmd"`. On Vast, use `--args` (CLI) or the `"args"` field (API):
...
curl -X POST "https://rest.runpod.io/v1/pods" \
    ...
    -d '{
        ...
        "ports": ["8000/http"],
        "dockerStartCmd": ["--model", "Qwen/Qwen3.5-4B", "--max-model-len", "4096"]
    }'
```
