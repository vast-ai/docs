...
<ParamField path="--sla_r_target" type="number">
  Client reliability coverage target (0–1). Values above 0 enable SLA on the offer. Legacy alias: `--sla_reliability`. (alias: `--sla-r-target`)
</ParamField>
...
<ParamField path="--sla_sigma_x" type="number">
  Host belief spread in log-failure space (0–3]. Lower = tighter confidence and lower client premiums. Defaults to `0.5`. (alias: `--sla-sigma-x`)
</ParamField>
