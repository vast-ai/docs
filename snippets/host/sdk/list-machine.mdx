...
<Note>
SLA listing parameters are **not yet** in the published Python SDK — use console **Set Pricing → SLA** or REST; see [SLA Offers](/host/sla-offers#api-for-automation). Volume/duration listing options (`-v`, `-z`, `-l`) are available in the **CLI** today; they are not documented on the REST `list_machine` / `create_asks` body.
</Note>
