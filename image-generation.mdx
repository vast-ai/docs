...
<script type="application/ld+json" dangerouslySetInnerHTML={{
  __html: JSON.stringify({
    ...
    "@type": "HowTo",
    "name": "How to Run Image Generation on Vast.ai",
    "description": "A complete guide to setting up and running image generation workloads including Stable Diffusion on Vast.ai's GPU marketplace, covering template selection, environment configuration, and optimization.",
    "step": [
      {
        "@type": "HowToStep",
        "name": "Select the Right Template",
        "text": "Navigate to the Templates tab and search for 'SD Web UI Forge' among recommended templates. This template comes pre-installed with the latest SD Web UI version, popular extensions, common models, and optimized settings for Vast.ai. It provides a user-friendly web interface with access to multiple models and extensions."
      },
      {
        "@type": "HowToStep",
        "name": "Configure Template Environment Variables",
        "text": "Edit the template and add/update key environment variables as needed: AUTO_UPDATE (auto-update to latest release), FORGE_REF (Git reference for updates), FORGE_ARGS (launch arguments), CF_TUNNEL_TOKEN (Cloudflare Zero Trust), CIVITAI_TOKEN (access gated Civitai models), HF_TOKEN (access gated HuggingFace models), and PROVISIONING_SCRIPT (URL to custom setup script). Never save your template as public if you've included tokens."
      },
      {
        "@type": "HowToStep",
        "name": "Choose an Instance",
        "text": "Select a GPU based on requirements: Minimum 8GB for basic models, 12GB+ recommended for larger models, 24GB+ for advanced techniques. GPU types: RTX 3090/4090 for best performance, RTX 3080/3080 Ti for good balance, A4000/A5000 for stability. Disk space: Minimum 50GB for base models, 100GB+ recommended for multiple models."
      },
      {
        "@type": "HowToStep",
        "name": "Connect to Your Instance",
        "text": "The Forge template provides multiple access methods: AI-Dock Landing Page (click Open button for automatic login), Direct Access with basic authentication (username: vastai, password: OPEN_BUTTON_TOKEN), API Access with Bearer token authorization, and Jupyter Access for uploading/downloading files. HTTPS and token authentication are enabled by default."
      },
      {
        "@type": "HowToStep",
        "name": "Manage Models and Generate Images",
        "text": "The template includes default provisioning script with base Stable Diffusion XL models, popular extensions, and common configurations. Create custom provisioning by modifying the script to download preferred models, extensions, and configurations. Organize models in /workspace/stable-diffusion-webui/models/ directories. Use the Web UI to configure settings, process batches, and generate images."
      }
    ]
  })
}} />
