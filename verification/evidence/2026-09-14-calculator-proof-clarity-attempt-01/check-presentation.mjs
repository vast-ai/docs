...
const attempt=process.argv[2];assert.match(attempt||'',/^\d{2}$/);assert(!fs.existsSync(dir+'/presentation-'+attempt+'.json'));
...
try{
 ...
 record('Offline card and separate calculator-output guidance',evaluate(`(()=>{
  ...
  return {id:'${id}',status:raw.status,text};
 })()`));
 run('screenshot',root+'/'+dir+'/offline-card-'+attempt+'.png');
 ...
 record('Live rendered card',evaluate(`(()=>{
  ...
  card.querySelector('.proof-guide').open=true;card.scrollIntoView();
  const text=card.innerText;for(const expected of ['Check calculator link','final URL, date and time, screenshot, and outcome','does not record a visit','Calculations','Market history','not requirements for closing this link entry'])if(!text.includes(expected))throw Error(expected);
  if(text.includes('console availability')||text.includes('agreement/Terms currency'))throw Error('Unrelated limits in calculator card');
  return {id:'${id}',status:card.dataset.currentStatus,text};
 })()`));
 run('screenshot',root+'/'+dir+'/localhost-card-'+attempt+'.png');
 result.status='PASS';
}catch(e){result.status='FAIL';result.error=String(e.stack);result.stdout=String(e.stdout||'');result.stderr=String(e.stderr||'');process.exitCode=1;}
finally{try{run('close');result.browserClosed=true;}catch(e){result.browserClosed=false;result.cleanupError=String(e);process.exitCode=1;}result.finished=new Date().toISOString();fs.writeFileSync(dir+'/presentation-'+attempt+'.json',JSON.stringify(result,null,2)+'\n',{flag:'wx'});console.log(JSON.stringify(result));}
