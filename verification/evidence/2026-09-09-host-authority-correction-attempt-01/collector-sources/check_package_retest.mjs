...
for(const p of files){let text=fs.readFileSync(p,'utf8');if(['scripts/host-review-html.test.mjs',dir+'/collector-sources/check_package.mjs',dir+'/collector-sources/check_package_retest.mjs'].includes(p)){const fixture='Authorization: Bearer exampleCredentialValue';assert.equal(text.split(fixture).length-1,1);text=text.replace(fixture,'[known synthetic sanitizer fixture]');fixtures.push({path:p,fixture:'exampleCredentialValue only'});}if(/--api-key[=\s]+[a-f0-9]{40,}|Authorization["':\s]+Bearer\s+[A-Za-z0-9_-]{20,}|-----BEGIN (?:OPENSSH |RSA |EC )?PRIVATE KEY-----/i.test(text))flagged.push(p);}
