# Host Docs: 31 corrections remaining

Prepared 14 September 2026 for CON-1518 / PR #185.

Four payout-provider passages have been corrected and checked within the supplied UI's scope. The remaining 31 entries are FAIL in the review model. They identify missing or insufficient support in the documentation, not 31 failed machine tests. Other pending reviews and the 23 prerequisite blockers remain separate. This walkthrough is navigation, not evidence or acceptance.
...
| Step | Topic | Entries |
| --- | --- | ---: |
| 1 | Payout options and setup guidance — corrected within scope | 0 |
...
## 1. Payout options and setup guidance — four corrected passages

Evidence received on 14 September: the user's Earnings screenshot shows all
three providers, and the user reports having used them. This supports the
provider list without another payout. No direct-transfer option is visible in
the crop, but that does not prove a universal ACH/wire/SWIFT prohibition.
...
The provider list now links to Earnings > Payout Account. The unsupported blanket
bank-transfer exclusion has been replaced with setup guidance. All four successor
passages are PASS only for those displayed provider options and checked source
links. No payment, fees, eligibility, active-account state or bank-transfer rule
was verified. Both reviewers show the narrowed wording and exact proof.
[Correction and retests](evidence/2026-09-14-payout-provider-correction-attempt-01/result.md).
...
**Next:** Move to payout and invoice rules below. Recheck the provider wording if the displayed options or Earnings route changes. A stronger statement about fees, bank routes or country availability needs its own source.

**Checked:** The three positive provider observations support the replacement setup instruction. The absence of a direct-transfer card was not used to prove a prohibition. The original wording and failures remain in the audit record.
...
### 4. Set up payouts through the displayed providers.
...
Audit reference: `MCL-9826b26393329d27` · current status: PASS for the replacement setup guidance, not the withdrawn bank-transfer exclusion. Both old FAQ fragments still resolve.
