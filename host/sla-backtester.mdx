...
Higher confidence (higher `p`) → tighter spread → lower simulated SLA charges for clients at a given target (more competitive `dph_total`). Tighter `p` does **not** reduce credit owed on a miss — use `worst_day_loss` and `reliability=` overrides to model settlement exposure.
