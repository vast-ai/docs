...
for id in $(vastai show machines -q); do
  ...
  # Preserve per-machine SLA tuning; defaults apply only when unset
  body=$(echo "$machine" | jq -c \
    '{machine: (.id|tonumber), price_gpu, end_date, min_chunk,
      sla_r_claim: (.sla_r_claim // 0.99),
      sla_sigma_x: (.sla_sigma_x // 0.5)}')
  if [ "$DRY_RUN" != "0" ]; then
...
Fetch the fleet once before the loop — do not call `show machines --raw` inside the loop. On fleets of 20+ machines, expect several minutes; back off on HTTP 429. Tune claim/confidence per machine (backtester + earnings) before setting `DRY_RUN=0`.
