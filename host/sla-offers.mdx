...
- While a rental runs they pay GPU, storage, bandwidth (usage-based), and a **small hourly SLA charge**. In search results, **`dph_total`** is the quoted hourly rental cost: GPU + storage + SLA charge at the quoted allocation — **not** bandwidth.
...
**Tighter confidence** (tighter spread; console label **Tight**) → lower client SLA charges and less settlement exposure for the host. **Wider confidence** (console label **Wide**) → higher client SLA charges and more exposure on bad days.
