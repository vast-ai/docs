...
- Make sure automatic updates can't replace the kernel or the NVIDIA driver, so that your machine doesn't drop a client job to update a driver. Security updates for the rest of the system should stay on, covered in [Automatic Security Updates](/host/security/automatic-security-updates).
