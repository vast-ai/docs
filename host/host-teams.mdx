...
5. Assign a role:
   - `manager` for people who need to install, register, list, unlist, reprice, maintain, or troubleshoot team-owned host machines.
   - `member` for people who should belong to the team but should not manage host machines.
6. Send the invite and confirm the operator appears on the Members page after accepting.
...
Host teams should grant machine access intentionally. The invite dialog currently exposes `member` and `manager` roles.

| Role | Host machine access |
| --- | --- |
| `member` | Should not be used for day-to-day host operations. Use this for people who need team membership but should not manage host machines. |
| `manager` | Use this for host operators who need to register and manage team machines, including listing, unlisting, price changes, min-bid changes, default jobs, cleanup, and maintenance. |
| Owner | Can manage machines like a Manager and can also administer or delete the team account. |

For the full permission reference, see [API Permissions](/api-reference/permissions). The exact console flow for granting machine registration rights to a team API key or custom role still needs engineering confirmation before this draft is published.
...
- Give day-to-day operators the `manager` role from the Members invite flow.
