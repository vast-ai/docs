...
**Save this output.** `Installed` is your rollback version. If `Candidate`
equals `Installed` for both, step 5 has nothing to do.
...
Save this too; the bracketed version is what you roll back to. Empty list and
Docker already current: nothing to upgrade. **Relist and stop here.**
...
Both must say `active`. Anything else: see
[Docker will not start](#docker-will-not-start) before continuing.
...
If the machine does not come back, use the out-of-band console. The kernel did
not change, so follow the
[kernel page's recovery steps](/host/upgrade-kernel#if-the-machine-does-not-come-back).
...
Both must say `active`. Otherwise see
...
One line per GPU, driver version as installed in step 6.
`Driver/library version mismatch` after the reboot: see
...
Only `ubuntu:22.04` or nothing: see the last row of the table in
[Docker will not start](#docker-will-not-start).
...
Otherwise see [The Vast daemon is not running](#the-vast-daemon-is-not-running).
...
The last `dockerd[...]` line before `Main process exited` is the error:

| Error contains | Fix |
| --- | --- |
| `unable to configure the Docker daemon with file /etc/docker/daemon.json` | Run `sudo dockerd --validate` to find the bad line. Fix it, then `sudo systemctl restart docker` |
| `failed to start containerd` / `containerd.sock: connect: no such file` | Run `sudo journalctl -u containerd -n 30 --no-pager`. On a config version error: `sudo containerd config migrate | sudo tee /etc/containerd/config.toml`, then `sudo systemctl restart containerd`, then `sudo systemctl restart docker` |
| `dpkg was interrupted` / `half-configured` | `sudo dpkg --configure -a`, then `sudo apt install -f`, then repeat step 5 |
| `error initializing graphdriver` / `driver not supported`, or `docker images` is empty | Remove any `storage-driver` or `containerd-snapshotter` line from `daemon.json`, then `sudo systemctl restart docker` |

If none apply, roll back to the `Installed` versions saved in step 2:
...
Docker restarts as part of this. Continue with step 6.

<Warning>
Only downgrade to the exact versions you came from. Do not delete
`/var/lib/docker`: it holds every cached image.
</Warning>

### Other packages broke something

Old versions are in the `apt list --upgradable` output from step 2 and in apt's
log:

```bash
grep -A3 'Start-Date' /var/log/apt/history.log | tail -n 20
```

```text nocopy
Start-Date: 2026-09-07  14:02:11
Commandline: apt upgrade
Upgrade: libvirt-daemon-system:amd64 (10.0.0-2ubuntu8.8, 10.0.0-2ubuntu8.9), ...
End-Date: 2026-09-07  14:09:40
```

Each entry is `(old, new)`. Roll one package back with
`sudo apt install --allow-downgrades <package>=<old_version>`, then repeat
step 8.

### Driver did not load after reboot

**Prebuilt (`linux-modules-nvidia-*`).** The module package for the running
...
Must start with `ii` (`rc` lines are old-kernel leftovers). If missing, install
it with your driver branch in place of `595-open`, then reboot:

```bash
sudo apt install linux-modules-nvidia-595-open-$(uname -r)
```

**DKMS (`nvidia-dkms-*`).** Rebuild, then reboot:
...
```text nocopy
Building module nvidia/595.98 for kernel 7.0.0-31-generic...
Installing module nvidia/595.98 for kernel 7.0.0-31-generic
```

A compiler error means the driver does not support this kernel yet: roll the
driver back (see [Other packages broke something](#other-packages-broke-something))
and contact Vast.ai Support with the output.

**Either type.** If the module is present but still does not load:
...
```text nocopy
NVRM: API mismatch: the client has the version 595.98, but
NVRM: this kernel module has the version 595.84.
```

`API mismatch`: run `sudo apt install --only-upgrade` on the
`linux-modules-nvidia-*` meta package from step 6 and reboot. Any other `NVRM`
error: contact Vast.ai Support with this output.

### The Vast daemon is not running

`sudo systemctl status vastai` must not say `Loaded: masked`; if it does, redo
the start commands in step 8. Then give it 5 minutes. If `nvidia-smi` fails, fix the driver first
(see [Driver did not load](#driver-did-not-load-after-reboot)); the daemon
recovers on its own once the GPUs work.

If `nvidia-smi` is healthy, follow the [Offline Machine](/host/machine-offline)
page from its step 2.
