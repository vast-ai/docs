...
Verification is fully automated. Proprietary algorithms continuously evaluate each machine's operational health and performance, dynamically adjusting assessments based on real-time **supply and demand**.
...
Passing self-test makes a machine eligible, but it does not guarantee immediate verification. Timing depends on automated checks, reliability, machine health, supply and demand, and platform policy. Support can help with account or backend anomalies, but ordinary host verification should not be treated as a manual support queue.
...
|---------------|------------------------|--------------------|
| Reliability | Staying above the 0.9 eligibility gate; higher reliability improves confidence | Proactive monitoring; steady power/thermals; minimize restarts |
