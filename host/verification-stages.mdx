...
- Dedicated machines only - the machine shouldn't be doing other stuff while
  ...
  when it runs.)
- Fast, reliable internet: 500 Mbps download / 500 Mbps upload recommended,
  with latency under 50 ms, jitter under 20 ms, and zero packet loss. Slower
  connections are allowed for listing but reduce rentability and may fail
  verification.
- NVIDIA GPU (Maxwell architecture or newer) with at least 4 GB of VRAM per
  GPU. NVIDIA driver version 580 or newer recommended.
- At least 2 physical CPU cores per GPU.
- Your CPU must support AVX instruction set (not all lower end CPUs support
  this).
...
- An open TCP/UDP port range mapped to each machine, within ports 1024-65535,
  containing at least (3 x GPU count + 2) ports, and reachable from the
  internet.
...
- Reliability of 90%
- At least 3 open ports per GPU plus 2 (100 recommended)
- Internet Download speed must scale with total machine VRAM at ~2.6 Mbps per
  GiB, with a 100 Mbps floor and 500 Mbps ceiling
- Internet Upload speed must scale with total machine VRAM at ~2.6 Mbps per
  GiB, with a 100 Mbps floor and 500 Mbps ceiling
- GPU RAM of 7 GB
