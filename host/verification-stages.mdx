...
| Forwarded port range | (3 x GPUs) + 2 contiguous ports, dedicated to rentals |
| Recommended range size | 100 ports per GPU |
...
<Warning>
**Dedicated machines only.** Any personal workload on the machine will cause
the Self-Test to fail, and the machine will not be verified.
</Warning>
