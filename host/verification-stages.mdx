...
<Note>
Networking drifts after it has been configured correctly. Links renegotiate to
a lower speed, a reboot moves the machine's LAN address out from under the
router's forwarding rule, and a machine that gains GPUs outgrows the port range
forwarded to it. See
[Network Maintenance](/host/network-maintenance) for the routine check against
these requirements.
</Note>
