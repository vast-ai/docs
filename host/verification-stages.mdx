...
[SSH Access](/host/security/ssh-access).
</Warning>

<Warning>
Use a separate SSH key pair for each machine you operate. One key reused across
your fleet means a single compromise exposes every machine you host. See
[SSH Access](/host/security/ssh-access#6-give-every-machine-its-own-key-pair)
for how to migrate a fleet that already shares one key.
</Warning>
