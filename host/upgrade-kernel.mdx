...
This page has two paths. The first upgrades the whole system, which is the
shorter route if you already keep the machine current. The numbered steps that
follow it update the kernel only: they do not move Ubuntu to a newer version,
for example 22.04 to 24.04, and they leave your other software alone.
...
rest of the system; the whole-system path below installs those as well, while
the numbered steps leave them alone.
</Note>
...
## Upgrading the whole system instead

If you already keep this machine's packages current, the kernel moves with
everything else and the numbered steps below collapse to three commands:
...
`linux-oem-22.04` on certified OEM hardware or `linux-nvidia`, where step 1
matches nothing.

<Warning>
This is a full system upgrade, not a kernel upgrade. Every package with an
update available moves inside the same maintenance window. The kernel-only
command in step 3 reports `24 not upgraded`, and that is the point of it; this
path upgrades those 24 as well. Docker, containerd and libvirt are upgraded and
their services restarted along with the rest, so if the machine misbehaves after
the reboot you have the whole upgrade to work through rather than one variable.

Take this path only if you are prepared to own that. If you want the kernel to
move on its own, use the numbered steps.
</Warning>
...
The rest of this page is the kernel-only path, and the recovery sections at the
end apply to both.
