...
<Warning>
A kernel upgrade only takes effect after a reboot, and a reboot destroys every
running instance on the machine. Do not start until you have a maintenance
...
Find which kernel metapackage is installed. This is the name you will upgrade in
step 3, and it differs between machines, so save it to a variable that the later
commands read:

```bash
KERNEL_METAPACKAGE=$(dpkg-query -W -f='${db:Status-Abbrev} ${Package}\n' 'linux-generic*' 2>/dev/null | awk '$1 == "ii" { print $2 }' | sort | tail -1); echo "$KERNEL_METAPACKAGE"
```
...
`KERNEL_METAPACKAGE` lives only in the shell session you set it in. If your
...
<Warning>
This is why the name is queried rather than hardcoded. On an HWE machine
...
```bash
apt-cache policy "$KERNEL_METAPACKAGE"
```
...
This is the comparison that decides whether you go on.
...
```bash
sudo apt install --only-upgrade "$KERNEL_METAPACKAGE"
```
...
<Warning>
This destroys every running instance on the machine. Before you run it, confirm
...
```bash
KERNEL_METAPACKAGE=$(dpkg-query -W -f='${db:Status-Abbrev} ${Package}\n' 'linux-generic*' 2>/dev/null | awk '$1 == "ii" { print $2 }' | sort | tail -1); apt-cache policy "$KERNEL_METAPACKAGE"
```
...
Start with the entry GRUB is configured to boot:
...
```bash
sudo cp /etc/default/grub /etc/default/grub.bak && sudo sed -i 's/^GRUB_DEFAULT=.*/GRUB_DEFAULT=0/' /etc/default/grub && grep '^GRUB_DEFAULT=' /etc/default/grub
```
...
Editing that file changes nothing on its own. Regenerate the menu:
...
If the new kernel is absent, the menu never had an entry for it, which is the
other way a machine boots the old kernel: the install wrote the kernel but the
menu was never rebuilt. `update-grub` is itself the fix for that.

<Warning>
Booting the corrected default takes another reboot, which destroys every running
