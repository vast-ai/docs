...
- On success: A new row is inserted into `user_tfa_methods` with `method="totp"`. `users.tfa_enabled` is set to `true`. Recovery codes returned if this is the first method.
