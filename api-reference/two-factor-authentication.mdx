...
### `tfa_enabled` (users table column — DB-only, NOT returned by any API)

The `tfa_enabled` boolean in the `users` database table indicates whether 2FA is active for the account. **This field is NOT returned by any API endpoint** — not by `show user`, not by `tfa/status/`, not by login. It exists only on the DB row and is consumed internally by backend logic.

If you need to inspect this value, query the database directly. Application code and CLI tools should use the `tfa_status` field on the user object (see below).
...
### `GET /api/v0/tfa/status/` endpoint (does NOT return enabled/disabled/legacy)

The `tfa/status/` endpoint returns a different shape than the `tfa_status` field above. It returns the user's configured methods, backup code count, and whether they are currently authorized to add a new method:
...
```json
{
  ...
  "authorized_to_add_method": false
}
```

It does **not** return the `enabled|disabled|legacy` enum — that comes from the `tfa_status` field on the user object (above), not from this endpoint.
