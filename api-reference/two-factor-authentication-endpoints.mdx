...
| `POST /api/v0/tfa/sms/resend/` | ✅ secret only (recovers lost UI tfa_secret if missing) | ✅ secret + bearer | ✅ bearer (less common, but accepted) |
...
Returns the full authenticated user object — the same shape as the `show user` endpoint.
...
- **CLI flow** (request authenticated via API key as Bearer): the response body **includes** a `session_key` field. CLI callers use this value as `Authorization: Bearer <session_key>` for subsequent authenticated requests.
...
- `backup_codes_remaining` — only included when the caller authenticated using a `backup_code` (i.e., a recovery code consumed this login). The value is the count of remaining unused backup codes after this consumption. Useful for prompting the user to regenerate codes when the count is low.
- `legacy_tfa_user: true` — only included when the user's account is in legacy 2FA state (signals the frontend to show a migration banner).
