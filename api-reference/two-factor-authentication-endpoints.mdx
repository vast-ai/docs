...
| `2fa_login_failed` | 400 | Generic 2FA login verification failure (e.g., SMS 2FA configured without a valid phone number on the account or method) |
...
| `sms_fraud_detected` | 403 | Account is permanently flagged for SMS pumping fraud (`billing_blacklisted=1`). SMS sending is disabled on the account. Contact support. |
