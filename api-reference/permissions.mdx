...
**Client SLA search → REST rent (minimum):**
...
SLA settlement credits are console-only for renters today — `billing_read` does not expose client SLA credit rows in `GET /api/v0/charges/`.
