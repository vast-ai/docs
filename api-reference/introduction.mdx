...
<Note>
Save the numeric `id` value. If you need to update your user profile, use
`PUT /api/v0/users/{id}/` with this ID. The alias `PUT /users/me/` is not supported
...
<Note>
**VRAM units:** `gpu_ram` and `gpu_total_ram` in the REST API are in **megabytes**.
To filter for 48 GB of VRAM, use `{"gte": 49152}`. The CLI (`vastai search offers`)
accepts gigabytes — `gpu_ram>=48` — and converts automatically.
</Note>
...
## 4. Register Your SSH Key (Required for SSH Access)

**Do this before creating an instance.** Account-level SSH public keys are applied at container creation time, but can also be added to a running instance via the SSH key button on the instance.

<Warning>
If you create an instance before registering your SSH key, you will not be able to SSH in.
You can add a key to a running instance without recreating it — use the SSH key button on the
instance card in the console, or via CLI: `vastai create ssh-key ~/.ssh/id_ed25519.pub`, or
via API: `POST /api/v0/instances/{id}/ssh/` with `{"ssh_key": "..."}`.
</Warning>
...
requests.post(f"{BASE_URL}/ssh/", headers=headers, json={"ssh_key": pubkey})
```
</CodeGroup>

Or with the CLI: `vastai create ssh-key ~/.ssh/id_ed25519.pub`

Your key persists on your account — you only need to do this once per key.
...
<Note>
The `onstart` field is limited to 4048 characters. See the [Template Settings](/documentation/templates/template-settings#on-start-script)
page for a gzip+base64 workaround for longer scripts.
</Note>
...
<Note>
`GET /instances/` (listing all instances) returns `{"instances": {"ID": {...}, ...}}` —
an **object keyed by instance ID**, not an array. When polling a specific instance by ID
(`GET /instances/{id}/`), the `instances` value is a single object (not a dict of dicts).
</Note>
