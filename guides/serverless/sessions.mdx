...
<Note>
  Sessions are a feature of the [Serverless SDK](./sdk-overview) and the PyWorker web server. Both sides are built in: any PyWorker built with `Worker`/`WorkerConfig` already serves the session routes described below, and no worker code is required to support basic session usage.
</Note>
...
- **The session's cost is excluded from queue-time calculations.** A worker holding idle sessions does not start rejecting fresh requests with `429` on the basis of reserved-but-unused capacity, because `max_queue_time` is evaluated against real in-flight requests only.
