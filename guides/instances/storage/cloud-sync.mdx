...
<Warning>
**WARNING**<br />
The S3 credentials inherit the permissions of the Hugging Face token they were generated from, across all Storage Buckets in every namespace (user and organizations) that token can access. Other repository types (models, datasets, Spaces) are not exposed through the S3 API. For sensitive use cases, generate the credentials from a fine-grained token scoped to only the namespaces and buckets you intend to sync.
</Warning>
...
5. You can create a bucket on your Hugging face account, which you would point your vast instance to later in the cloud copy ui on vast.ai [instance page](https://cloud.vast.ai/instances/)

6. Navigate to your Vast [settings](https://cloud.vast.ai/settings/) page and select **Connect Hugging Face**.
