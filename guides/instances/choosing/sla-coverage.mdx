...
3. Optionally filter to SLA-enabled offers (`has_sla` / SLA-enabled control) if available — separate from **Reliability target**.
...
| Offer gone / not rentable | HTTP 404/410 `no_such_ask` | Re-search and rent a current offer ID |
| Price mismatch after stale quote | HTTP 400 `invalid_price` | Re-search with the same `target_reliability` and storage, then rent again |
