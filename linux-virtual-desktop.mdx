...
<script type="application/ld+json" dangerouslySetInnerHTML={{
  __html: JSON.stringify({
    ...
    "@type": "HowTo",
    "name": "How to Set Up Linux Virtual Desktop on Vast.ai",
    "description": "A comprehensive guide to setting up and using a Linux Virtual Desktop environment on Vast.ai with GPU acceleration, including Ubuntu Desktop setup, remote access options, and advanced features.",
    "step": [
      {
        "@type": "HowToStep",
        "name": "Create Your Instance",
        "text": "Navigate to the Templates tab and search for 'Ubuntu Desktop (VM)' across all templates. Select the template by clicking the play button. Choose your preferred GPU from search results (try to find a GPU close to you). Click 'Rent' to create your instance. Go to Instances tab and wait for the blue button to say 'Open'. It can take time to load if the docker image isn't cached on the machine."
      },
      {
        "@type": "HowToStep",
        "name": "Access Your Instance",
        "text": "Connect using Browser Access (Recommended - click 'Open' button to launch Instance Portal, choose between Selkies WebRTC for better performance or NoVNC as alternative), VNC Client (connect to instance_ip:5900 with password from OPEN_BUTTON_TOKEN), or SSH Access (connect via SSH using command from console, or for non-root access: ssh -p mapped_port user@instance_ip)."
      },
      {
        "@type": "HowToStep",
        "name": "Perform First-Time Setup",
        "text": "Change the default password by executing 'passwd' command in Linux terminal (default username: user, default password: password) and follow the prompts. Optionally configure TLS by installing the 'Jupyter' certificate following the instance setup guide to eliminate certificate warnings in your browser."
      },
      {
        "@type": "HowToStep",
        "name": "Explore Pre-installed Software",
        "text": "The environment comes with Web Browsers (Firefox, Chrome), Development Tools (Docker for non-root use, terminal emulator, development utilities), Creative Software (Blender, Wine), and Gaming Support (Steam with Proton, Sunshine streaming server). All software is ready to use immediately."
      },
      {
        "@type": "HowToStep",
        "name": "Configure Advanced Features (Optional)",
        "text": "Set up Tailscale Integration for secure access (install Tailscale on local device, run 'sudo tailscale up' on instance). Configure Game Streaming with Moonlight (requires Tailscale, configure Sunshine server, connect using Moonlight client). Create Cloudflare Tunnels for secure access without exposing ports, managed via Instance Portal."
      }
    ]
  })
}} />
...
<Warning>
VM instances currently have a ~25% creation failure rate and longer boot times (5–30 minutes) compared to Docker-based instances. If your workload doesn't require a full desktop GUI, use a [Docker-based instance](/documentation/instances/docker-environment) instead.
</Warning>
