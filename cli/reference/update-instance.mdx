...
<ParamField path="--disk" type="number">
...
Use `--disk` to dynamically grow the container disk on a running or stopped instance without recreating it. The change is applied in place; existing data is preserved.
...
```bash
vastai update instance 1234 --disk 80
```
