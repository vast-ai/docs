...
<Warning>
**SLA coverage:** `target_reliability` is not available on this CLI command. Searching with a reliability target only quotes pricing — use the web console or REST create-instance to enable SLA. See [SLA Coverage](/guides/instances/choosing/sla-coverage#finding-sla-offers).
</Warning>
...
<ParamField path="id" type="integer" required>
  Offer/ask ID from search offers (the value passed to create-instance / `PUT /api/v0/asks/{id}/`). This is not a running instance ID.
</ParamField>
...
If you use args/entrypoint launch mode, we create a container from your image as is, without attempting to inject SSH or Jupyter.
...
# create an on-demand instance with a private image (use your registry password, not a real secret in docs)
vastai create instance 384827 --image YOUR_REGISTRY/pytorch:latest --login '-u YOUR_USER -p YOUR_PASSWORD docker.io' --jupyter --direct --env '-e TZ=PDT -e XNAME=XX4 -p 22:22 -p 8080:8080' --disk 20
...
Return value — JSON reporting the instance ID of the newly created instance:
