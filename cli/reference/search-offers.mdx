...
| `expected_reliability` | float | host SLA claim (`sla_r_claim`); `0` when SLA is off |
| `has_sla` | bool | restrict to offers with SLA enabled (`true`) or without (`false`) |
| `target_reliability` | float | your desired SLA coverage level; drives SLA charge in `dph_total` |
