...
Available fields:
    ...
    gpu_ram:                float     per GPU RAM in GB (GB in CLI; MB in raw REST API — the CLI converts automatically)
    gpu_total_ram:          float     total GPU RAM in GB (GB in CLI; MB in raw REST API — the CLI converts automatically)
