...
<ParamField path="--new" type="boolean">
  Use the experimental search path (internal / unstable). Prefer the default search for production and SLA filtering.
</ParamField>
...
| `target_reliability` | float | your desired SLA coverage level (greater than 0 and less than 1); drives SLA charge in `dph_total` (see [SLA Coverage](/guides/instances/choosing/sla-coverage)) |
