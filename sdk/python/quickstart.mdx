    ...
    "step": [
      {
        ...
        "text": "Install the latest stable PyPI release using pip: 'pip install vastai'. This installs the vastai package which provides convenient Python access to the Vast.ai API."
      },
      {
        "@type": "HowToStep",
        "name": "Import and Initialize the SDK",
        "text": "Import the VastAI class from the package: 'from vastai import VastAI'. Then construct a Vast client with your API key: 'vast_sdk = VastAI(api_key=\"YOUR_API_KEY\")'. This creates a client object that provides access to all Vast.ai resources."
      },
      {
        "@type": "HowToStep",
      ...
      },
      {
        ...
        "text": "Use additional SDK features: Copy files with 'vast_sdk.copy(src=\"source_path\", dst=\"destination_path\")'. Manage SSH keys with 'vast_sdk.create_ssh_key(ssh_key=\"your_ssh_key\")', 'vast_sdk.show_ssh_keys()', and 'vast_sdk.delete_ssh_key(ID=123456)'. Access API key management, endpoints, teams, host management, and more through respective methods."
      }
    ]
  })
}} />

We provide a [PyPI package](https://pypi.org/project/vastai/), `vastai`, for convenient Python usage.

<Note>`pip install vastai` installs both the `vastai` CLI and the Python SDK — the same package powers `vastai ...` on the command line and `from vastai import VastAI` in Python.</Note>
...
```text Text
vast_sdk.search_offers(query='gpu_name=RTX_5090 rentable=True')
```
...
```text Text
vast_sdk.copy(src='source_path', dst='destination_path')
```
...
Serverless endpoints, workergroups, and Deployments:

| Method | Description |
| ------ | ----------- |
| `create_endpoint(**kwargs)` | Create a new endpoint group. |
| `update_endpoint(id: int, **kwargs)` | Update an endpoint group. |
| `delete_endpoint(id: int)` | Delete an endpoint group. |
| `show_endpoints()` | Show all endpoint groups. |
| `get_endpt_logs(id: int, level: int = 1, tail: Optional[int] = None)` | Fetch logs for a serverless endpoint. |
| `create_workergroup(**kwargs)` | Create a new workergroup. |
| `show_workergroups()` | Show all workergroups. |
| `delete_workergroup(id: int)` | Delete a workergroup. |
| `get_wrkgrp_logs(id: int, level: int = 1, tail: Optional[int] = None)` | Fetch logs for a workergroup. |
| `show_deployments()` | Show all deployments. |
| `show_deployment(id: int)` | Show details for a deployment. |
| `show_deployment_versions(id: int)` | List versions for a deployment. |
| `delete_deployment(id: int)` | Delete a deployment. |

### File Management

| Method | Description |
| ------ | ----------- |
| `copy(src: str, dst: str)` | Copy files between instances. |
...
| `invite_member(email: str, role: str)` | Invite a new member to the team. |
| `remove_member(id: int)` | Remove a member from the team. |
| `create_team_role(name: str, permissions)` | Create a new team role. |
| `remove_team_role(name: str)` | Remove a role from the team. |
| `update_team_role(id: int, **kwargs)` | Update details of a team role. |
