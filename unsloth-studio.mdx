...
Once the instance shows **Running** status, find the connection details in your instance row. The Unsloth template exposes:

- **Port 8888** — Unsloth Studio web UI
- **Port 8080** — Jupyter Lab
- **Port 6006** — TensorBoard

Click the instance row to expand it, then open the **IP & Port Info** dialog to find your port mappings.

<Frame caption="Instance IP & Port Info showing the mapped ports for each service">
  <img src="/images/unsloth-studio/port-info.png" />
</Frame>

Find the port mapped to **8888** (Unsloth Studio) and copy the URL (`http://<INSTANCE_IP>:<PORT>`). Open it in your browser.

<Warning>
  Vast.ai maps container ports to different host ports. Use the IP and port shown in the Port Info dialog — do not connect to port 8888 directly.
</Warning>

### Instance Authentication

Your browser will prompt for HTTP credentials. These are the Vast.ai instance credentials, not your Vast.ai account login. To find them, click the **LOG** button on your instance:

<Frame caption="Click the LOG button to view instance logs and find your credentials">
  <img src="/images/unsloth-studio/instance-logs-button.png" />
</Frame>

Search the logs for:

```text Text
Your web credentials are: vastai / <PASSWORD>
```

The username is `vastai` and the password is the hex string shown on that line.

### Studio Account Setup

After passing the instance authentication, Studio asks you to set a password for your Studio account. This is a separate login stored only on the instance.
