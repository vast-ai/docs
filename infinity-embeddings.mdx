...
<script type="application/ld+json" dangerouslySetInnerHTML={{
  __html: JSON.stringify({
    ...
    "@type": "HowTo",
    "name": "How to Serve Infinity Embeddings on Vast.ai",
    "description": "A guide to deploying and serving embedding models using Infinity Embeddings framework on Vast.ai, including single and multiple model deployments, rerankers, and classifiers.",
    "step": [
      {
        "@type": "HowToStep",
        "name": "Install Vast CLI and Set API Key",
        "text": "Install the latest vastai package with 'pip install --upgrade vastai'. Create your account, then set your API key by running 'vastai set api-key <Your-API-Key-Here>'. You can find your API key on the Vast.ai CLI page."
      },
      {
        "@type": "HowToStep",
        "name": "Search for Suitable Machines",
        "text": "For serving an LLM, search for machines with static IP address, available ports, and a single modern GPU with decent RAM. Run 'vastai search offers 'compute_cap > 800 gpu_ram > 20 num_gpus = 1 static_ip=true direct_port_count > 1'' to query the API for suitable machines."
      },
      {
        "@type": "HowToStep",
        "name": "Deploy Single Embedding Model",
        "text": "Deploy a single model using the command line. Copy a specific instance id from the search results and create an instance with: 'vastai create instance <instance-id> --image michaelf34/infinity:latest --env '-p 8000:8000' --disk 40 --args v2 --model-id michaelfeil/bge-small-en-v1.5 --port 8000'. This deploys the Infinity image with the specified embedding model."
      },
      {
        "@type": "HowToStep",
        "name": "Connect and Test the Deployment",
        "text": "Once your instance is setup, click the highlighted button to see the IP address and correct port. Install OpenAI SDK with 'pip install openai'. Test the deployment by creating an OpenAI client pointing to your instance: 'client = OpenAI(api_key=\"EMPTY\", base_url=\"http://<Instance-IP-Address>:<Port>/v1\")' and calling 'client.embeddings.create(model=\"michaelfeil/bge-small-en-v1.5\", input=\"What is Deep Learning?\")' to get embeddings."
      },
      {
        "@type": "HowToStep",
        "name": "Deploy Multiple Models (Advanced)",
        "text": "Deploy rerankers, classifiers, and multiple models simultaneously by adding multiple --model-id arguments: 'vastai create instance <instance-id> --image michaelf34/infinity:latest --env '-p 8000:8000' --disk 40 --args v2 --model-id mixedbread-ai/mxbai-rerank-xsmall-v1 --model-id SamLowe/roberta-base-go_emotions --port 8000'. Use the requests library to call /rerank and /classify endpoints with appropriate payloads."
      }
    ]
  })
}} />
